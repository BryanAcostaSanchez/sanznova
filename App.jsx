...
function Sec({ children, bg = C.white, id }) {
  ...
  const padding = size.isMobile ? "40px 20px" : "80px 64px";
  const marginTop = size.isMobile ? 40 : 80;
  return (
    <section id={id} style={{ minHeight: "100vh", backgroundColor: bg, display: "flex", flexDirection: "column", justifyContent: "center", padding, boxSizing: "border-box", marginTop }}>
      {children}
    </section>
  );
}
...
function ResponsiveGrid({ children, cols = { mobile: 1, tablet: 2, desktop: 2 }, gap = { mobile: 12, desktop: 20 } }) {
  const size = useWindowSize();
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${numCols}, 1fr)`, gap: gapVal }}>
      {children}
    </div>
  );
}
...
export default function Brandbook() {
  ...
  return (
    <div style={{ fontFamily: F.sans, background: C.white, color: C.black, paddingTop: 50 }}>

      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", justifyContent: size.isMobile ? "space-between" : "center", alignItems: "center", gap: size.isMobile ? 0 : 20, padding: size.isMobile ? "12px 16px" : "14px 16px", background: "rgba(255,255,255,0.88)", backdropFilter: "blur(20px)", borderBottom: `1px solid ${C.g100}`, flexWrap: size.isMobile ? "nowrap" : "wrap" }}>
        ...
        {size.isMobile && (
          <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", cursor: "pointer", padding: "8px 4px" }}>
            <div style={{ width: 24, height: 16, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <div style={{ width: "100%", height: 2, background: C.black, borderRadius: 1, transition: "0.3s" }} />
              <div style={{ width: "100%", height: 2, background: C.black, borderRadius: 1, transition: "0.3s" }} />
              <div style={{ width: "100%", height: 2, background: C.black, borderRadius: 1, transition: "0.3s" }} />
            </div>
          </button>
      ...
      {size.isMobile && menuOpen && (
        <div style={{ position: "fixed", top: 50, left: 0, right: 0, zIndex: 99, background: C.white, borderBottom: `1px solid ${C.g100}`, maxHeight: "calc(100vh - 50px)", overflowY: "auto" }}>
          {nav.map((t, i) => (
            <a key={t} href={`#s${i}`} onClick={e => { e.preventDefault(); navClick(i); }}
              style={{ display: "block", padding: "14px 16px", fontFamily: F.mono, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: C.g400, textDecoration: "none", cursor: "pointer", borderBottom: `1px solid ${C.g100}` }}
              onMouseEnter={e => e.target.style.color = C.black} onMouseLeave={e => e.target.style.color = C.g400}>
              {t}
            </a>
          ))}
